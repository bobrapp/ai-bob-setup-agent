...
class EmailAgent(BaseAgent):
    ...
    def _call_llm_classify(self, email: EmailMessage) -> tuple[str, float]:
        ...
        from src.personal_foundation.llm_client import classify_email

        try:
            result = classify_email(email.sender, email.subject, email.body_preview)
    ...
    def _call_llm_draft(self, email: EmailMessage) -> str:
        ...
        from src.personal_foundation.llm_client import draft_email_reply

        try:
            return draft_email_reply(email.sender, email.subject, email.body_preview)
        except Exception:
    ...
    def _classify_outreach_response(self, email: EmailMessage) -> str:
        ...
        from src.personal_foundation.llm_client import classify_outreach_response

        try:
            result = classify_outreach_response(email.sender, email.subject, email.body_preview)
